          ...
          10:
            begin
              ...
              if (cbUchastok.ItemIndex + 1) = 11 then
                detail_filtr := 'id_uch<>1 and id_uch<>6 and id_uch<>9';
              //отдел сетевых коммуникаций
              if (cbUchastok.ItemIndex + 1) = 10 then
                detail_filtr := 'id_uch=1 or id_uch=6';
            end;
  ...
  *)
  If id_alert_uchastok = 2 then
    case cbShowMobila.ItemIndex of {
      0: Main_filtr := '((id_uch='+IntToStr(id_alert_uchastok)+' or id_uch=9) and if_mobila=1)';
      1: Main_filtr := '((id_uch='+IntToStr(id_alert_uchastok)+' or id_uch=9) and if_mobila=0)';
      2: Main_filtr := '(id_uch='+IntToStr(id_alert_uchastok)+' or id_uch=9)';
      3: Main_filtr := '((id_uch='+IntToStr(id_alert_uchastok)+' or id_uch=9) and if_malocenka=1)';
      4: Main_filtr := '((id_uch='+IntToStr(id_alert_uchastok)+' or id_uch=9) and if_malocenka=0)';
      5: Main_filtr := '((id_uch='+IntToStr(id_alert_uchastok)+' or id_uch=9) and if_mebel=1)';
      6: Main_filtr := '((id_uch='+IntToStr(id_alert_uchastok)+' or id_uch=9) and if_orgtehnics=1)';
      7: Main_filtr := '((id_uch='+IntToStr(id_alert_uchastok)+' or id_uch=9) and if_phone=1)';
      8: Main_filtr := '((id_uch='+IntToStr(id_alert_uchastok)+' or id_uch=9) and (if_phone=0) and (if_orgtehnics=0) and (if_mebel=0) and (if_malocenka=0) and (if_mobila=0))';
      }
    ...
    if id_alert_uchastok=7 then
      case cbShowMobila.ItemIndex of
        0: Main_filtr := '(id_uch<10 and if_mobila=1)';
        1: Main_filtr := '(id_uch<10 and if_mobila=0)';
        2: Main_filtr := '(id_uch<10)';
        3: Main_filtr := '(id_uch<10 and if_malocenka=1)';
        4: Main_filtr := '(id_uch<10 and if_malocenka=0)';
        5: Main_filtr := '(id_uch<10 and if_mebel=1)';
        6: Main_filtr := '(id_uch<10 and if_orgtehnics=1)';
        7: Main_filtr := '(id_uch<10 and if_phone=1)';
        8: Main_filtr := '((id_uch<10) and (if_phone=0) and (if_orgtehnics=0) and (if_mebel=0) and (if_malocenka=0) and (if_mobila=0))';
      end
    else
      case cbShowMobila.ItemIndex of {
        0: Main_filtr := '(id_uch='+IntToStr(id_alert_uchastok)+' or id_uch=9)';
        1: Main_filtr := '((id_uch='+IntToStr(id_alert_uchastok)+' or id_uch=9) and if_malocenka=1)';
        2: Main_filtr := '((id_uch='+IntToStr(id_alert_uchastok)+' or id_uch=9) and if_malocenka=0)';
        3: Main_filtr := '((id_uch='+IntToStr(id_alert_uchastok)+' or id_uch=9) and if_mebel=1)';
        4: Main_filtr := '((id_uch='+IntToStr(id_alert_uchastok)+' or id_uch=9) and if_orgtehnics=1)';
        5: Main_filtr := '((id_uch='+IntToStr(id_alert_uchastok)+' or id_uch=9) and if_phone=1)';
        6: Main_filtr := '((id_uch='+IntToStr(id_alert_uchastok)+' or id_uch=9) and (if_phone=0) and (if_orgtehnics=0) and (if_mebel=0) and (if_malocenka=0))';
        }
        2: Main_filtr := '(id_uch='+IntToStr(id_alert_uchastok) +')';
...
begin
  //выбираем активную базу
  TMP_Data_Set := gridTMC.Datasource.Dataset;
  //если нажали копировать а записей нет то выход
  if (TMP_Data_Set = nil)or(TMP_Data_Set.RecordCount=0) then exit;
  frmAddEditTMC := TfrmAddEditTMC.Create(Application);
  frmAddEditTMC.Caption := 'Основные средства';
  UAddEditTMC.Data_Set2 := TSQLClientDataSet(TMP_Data_Set);
  flag_copy := true;
  frmAddEditTMC.Caption := frmAddEditTMC.Caption + ' (добавление)';
  flag_edit2 := true;
  frmAddEditTMC.ShowModal;
end;

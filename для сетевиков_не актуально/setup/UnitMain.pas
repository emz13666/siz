...
begin
  ...
  begin
    parametres := '"\\10.70.0.105\siz_install\siz.exe"';
    reg := TRegistry.Create;
    ...
    reg.WriteString('SIZ','\\10.70.0.105\siz_install\siz.exe');
    reg.CloseKey;
    reg.Free;
  end;
  ...
  if CheckBox1.Checked then
    CreateShortcut('\\10.70.0.105\siz_install\siz.exe', _DESKTOP, '','','','—истема контрол€ отдела св€зи');

   Form1.Hide;
   ShowMessage('”становка успешно завершена.');
   {запуск siz.exe}
  if CheckBox3.Checked then
     WinExec(Pchar('\\10.70.0.105\siz_install\siz.exe'), SW_SHOW);

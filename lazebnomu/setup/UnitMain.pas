...
begin
  ...
  if CheckBox1.Checked then
    CreateShortcut('c:\program files\siz.exe', _DESKTOP, '','','','Система контроля отдела связи');

   Screen.Cursor :=  crDefault;
   Form1.Hide;
   ShowMessage('Установка программной оболочки успешно завершена.'+#13+'Установлено в c:\program files\siz.exe');
   {запуск siz.exe}
...
begin
  ...
  begin
    ...
    reg.RootKey := HKEY_LOCAL_MACHINE;
    reg.OpenKey('SOFTWARE\Microsoft\Windows\CurrentVersion\Run',false);
    reg.WriteString('Start_Mysql_SIZ','C:\mysql_siz\start_local.cmd');
    reg.CloseKey;
    reg.Free;
  end;
  ...
  begin
    CreateShortcut('C:\mysql_siz\start_local.cmd', _DESKTOP, '','','','Запуск СУБД MySQL');
    CreateShortcut('C:\mysql_siz\stop_sql.cmd', _DESKTOP, '','','','Остановка СУБД MySQL');
  end;

   {запуск MySql}
  if CheckBox3.Checked then
     WinExec(Pchar('C:\mysql_siz\start_local.cmd'), SW_SHOW);
  Form1.Hide;
  Screen.Cursor :=  crDefault;
   ShowMessage('Установка СУБД MySQL успешно завершена.'+#13+'Установлено в c:\mysql_siz\');
   Application.Terminate;
end;

...
var win_dir,tmpdir:string;
    ...
    fSND_LOOP: boolean;//зациклить воспроизведение звука
    fileSound: string = ''; //имя файла для воспроизведения звука. Если пустой - то по умолчанию.
    playedSound: boolean;//воспроизводится ли звук в данный момент (эту переменную использует поток ThreadBeep)

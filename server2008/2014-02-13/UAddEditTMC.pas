...
begin
  ...
  if (cbUchastok.ItemIndex+1<>11) then
   if (id_alert_uchastok < 7)and(id_alert_uchastok <> cbUchastok.ItemIndex+1)and(cbUchastok.ItemIndex+1<>9)and(cbUchastok.ItemIndex+1<>10) then
   begin
...
begin
  ...
  if id_alert_uchastok = 11 then cbUchastok.ItemIndex := 10;
...
procedure TfrmAddEditTMC.FormCreate(Sender: TObject);
begin
  DM.sl_query.Close;
  dm.sl_query.CommandText := 'uchastok';
  dm.sl_query.Open;
  DM.sl_query.First;
  cbUchastok.Items.Clear;
  while not DM.sl_query.Eof do
  begin
    cbUchastok.Items.Add(DM.sl_query.Fields[1].AsString);
    DM.sl_query.Next;
  end;
  DM.sl_query.Close;
end;

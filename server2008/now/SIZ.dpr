...
begin
 ...
 // а если есть - загружаем из реестра
   ...
   if reestr.ValueExists('SND_LOOP') then
     fSND_LOOP := reestr.ReadBool('SND_LOOP')
   else
   begin
     fSND_LOOP := true;
     try
     reestr.WriteBool('SND_LOOP',fSND_LOOP);
     except
     end;
   end;
   ...
   begin
     prodolgitelnost := 90;
     try
     reestr.WriteInteger('prodolgitelnost',90);
     except
     end;
   end;
   if prodolgitelnost < 10 then prodolgitelnost := 10;
   reestr.CloseKey;
   reestr.Free;

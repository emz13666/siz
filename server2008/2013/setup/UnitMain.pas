...
begin
  ...
  begin
    parametres := '"\\SERVER2008\Зиновьев\SIZ\siz.exe"';
    reg := TRegistry.Create;
    ...
    reg.WriteString('SIZ','\\SERVER2008\Зиновьев\SIZ\siz.exe');
    reg.CloseKey;
    reg.Free;
  end;
  ...
  if CheckBox1.Checked then
    CreateShortcut('\\SERVER2008\Зиновьев\SIZ\siz.exe', _DESKTOP, '','','','Система контроля отдела связи');

   Form1.Hide;
   ShowMessage('Установка успешно завершена.');
   {запуск siz.exe}
  if CheckBox3.Checked then
     WinExec(Pchar('\\SERVER2008\Зиновьев\SIZ\siz.exe'), SW_SHOW);

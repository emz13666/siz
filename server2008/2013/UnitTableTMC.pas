...
--Участок*)
  ...
  begin
    ...
    cbUchastok.Items.Add('Техбюро');
    cbUchastok.Items.Add('Участок АСУГТК');
...
begin
    if id_alert_uchastok=7 then Main_filtr := '(id_uch<12)'
      else
    ...
    begin
      ...
      //все остальные категории - если не одной галочки не стоит
      if flOther=1 then
          if id_alert_uchastok=7 then   Main_filtr := Main_filtr + ' or (id_uch<12 and if_mobila=0 and if_mebel=0 and if_phone=0 and if_orgtehnics=0)'
      ...
      if flSpisano=1 then Main_filtr := '('+Main_filtr+ ') and Sostoyanie=''Списано'''
        else Main_filtr := '('+Main_filtr+ ') and Sostoyanie<>''Списано''';
    end;
